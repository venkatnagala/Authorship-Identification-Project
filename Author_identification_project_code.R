...
#interactive.mode.with.GUI = FALSE

#######  TEXT- AND LANGUAGE-DEPENDENT SETTINGS  ####################


# format of corpus files; available choice:
# "plain"
corpus.format = "plain"
...
# or Support Vectors Machines ("svm")
...
# Theoretically, one can switch on every option at once (not much sense 
# in that). This will work, because the script will treat the last switched on 
# option as your actual choice. You may also want to switch off each option 
# (then, the logfile will be the only output).
...
# Do you want the graphs coloured? Do you want titles on the graphs?
...
# Given a language option ("English", as described 
...
# case the default distance (Manhattan distance) will be switched on. Be aware
# of correct spelling: then the default value will be assigned as well!

if(distance.measure %in% c("MH","EU") == FALSE) {
  distance.measure = "MH"
  }
...
.Tcl("option add *font myDefaultFont")  
  cancel_pause <- FALSE
  tt <- tktoplevel()
  tktitle(tt) <- "Enter Parameters for Authorship Identification Project"
...
#
tkgrid(tklabel(tt,text="  INPUT:           "),entrylabel_TXT,sticky="w")
tkgrid(tklabel(tt,text="                   "),entry_TXT,sticky="w")
#  
# next row: LANGUAGE
#
...
#
tkgrid(tklabel(tt,text="  LANGUAGE:        "),entrylabel_EN3,sticky="w")
tkgrid(tklabel(tt,text="                   "),entry_EN3,sticky="w")
#  
...
#
#
tkgrid(tklabel(tt,text="  FEATURES:        "),entrylabel_W,entrylabel_L,entrylabel_NGRAMSIZE,cblabel_RAND,entrylabel_SIZE,sticky="w")
tkgrid(tklabel(tt,text="                   "),entry_W,entry_L,entry_NGRAMSIZE,cb_RAND,entry_SIZE,sticky="w")
...
#
#
tkgrid(tklabel(tt,text="  CLASSIFIER:      "),entrylabel_KNN,entrylabel_SVM,entrylabel_NBAYES,sticky="w")
tkgrid(tklabel(tt,text="                   "),entry_KNN,entry_SVM,entry_NBAYES,sticky="w")
...
# next row: Distance Calculation OPTIONS
#
...
#
tkgrid(tklabel(tt,text=" DISTANCE:         "),entrylabel_MH,entrylabel_EU,sticky="w")
tkgrid(tklabel(tt,text="                   "),entry_MH,entry_EU,sticky="w")
...
split.into.words = function(input.text) {
  ...
  ### Linux, Mac
  #tokenized.txt = c(unlist(strsplit(input.text, "[^[:alpha:]]+")))
  ### Windows
  tokenized.txt = c(unlist(strsplit(input.text, "\\W+",perl=T)))
...
#    }    
  ...
  if(corpus.lang == "English" || corpus.lang == "English.all") {
    # replacing non-ASCII apostrophes with simple ' (standard ASCII char)
    tokenized.sample = gsub("’","'",input.text)
...
#      }
    if(corpus.lang == "English.all" || corpus.lang == "English") {
      tokenized.sample=c(unlist(strsplit(tokenized.sample,"[^[:alpha:]^-]+")))
...
make.ranking.of.candidates = function(dist.matrix,candidates) {
  ...
  current.sample = c(gsub("_.*","",colnames(dist.matrix)))[ranked.candidates]
    if((gsub("_.*","",rownames(dist.matrix)))[h] != current.sample[1]) {
      ...
      current.sample[1:candidates], " (score:", 
...
# here begins the code for SVM, kNN, NaiveBayes
#

# obsolete value, somewhere still needed
...
if(tolower(classification.method) == "knn") {
  ...
  classes.training = gsub("_.*","",rownames(primary.set))
  training.set = cbind(classes.training,primary.set[,1:mfw])
  classes.test = gsub("_.*","",rownames(secondary.set))
  test.set = cbind(classes.test,secondary.set[,1:mfw])
  #
...
#     cat("\nMisclassified Samples:",file=outputfile,append=T)
     cat("\n\n\n",file=outputfile,append=T)   
...
#    
      Wellclassified.samples = 
        paste(rownames(test.set), "\t-->\t",
              classification.results)[classes.test==classification.results]
     cat(Wellclassified.samples,file=outputfile,append=T,sep="\n")
    }
...
do.not.remove = c("freq.table.both.sets",
